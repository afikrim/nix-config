...
__git_worktree_boon_core() {
  ...
  echo "→ Restoring database.yml from aziz/local-setup…"
  git show aziz/local-setup:packages/boon-core/config/database.yml > config/database.yml || return 1

  echo "→ Restoring .serena setup from $BOONAI_DEV_DIR/boon/packages/boon-core/.serena"
  cp -r $BOONAI_DEV_DIR/boon/packages/boon-core/.serena .serena
