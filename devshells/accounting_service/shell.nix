            ...
            perl -0pi -e 's#logs/gc.log#'"$ELASTICSEARCH_LOG_DIR"'/gc.log#g' "$ELASTICSEARCH_CONFIG_DIR/jvm.options"
              ...
              if [ ! -f "$KAFKA_LOG_DIR/meta.properties" ]; then
                echo "Initializing Kafka storage in $KAFKA_LOG_DIR"
                  ...
                  kafka-storage.sh format --ignore-formatted --cluster-id "$CLUSTER_ID" --config dev/services/kafka/server.properties >/dev/null
                  fi
    ...
    export ES_LOG_DIR="$ELASTICSEARCH_LOG_DIR"
      ...
      if [ -z "$ACCOUNTING_SERVICE_NIX_SHELL" ]; then
        export ACCOUNTING_SERVICE_NIX_SHELL=1
          cat <<'EOT'
          Dev shell ready. Helpful commands:
          process-compose --config dev/process-compose.yaml up services
# starts MySQL, Redis, Elasticsearch, ZooKeeper, Kafka

          go test ./...
          make test / make lint / make prepare

          Data for stateful services is stored under .devdata/.
          EOT
          fi
          '';
  };
